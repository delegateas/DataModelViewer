...
```
DataModelViewer/
├── Generator/              # C# .NET 8 - Metadata extraction & analysis
│   ├── DTO/               # Data Transfer Objects (entities, attributes, etc.)
│   ├── Services/          # Plugin, Flow, WebResource analyzers
│   ├── Queries/           # Dataverse query definitions
│   ├── DataverseService.cs    # Main Dataverse interaction
│   ├── WebsiteBuilder.cs      # Generates Data.ts
│   └── Program.cs             # Entry point
│
├── Website/               # Next.js 15 - Frontend application
│   ├── app/              # Next.js App Router
│   │   ├── api/          # API routes (auth, diagram CRUD)
│   │   ├── diagram/      # Diagram editor page
│   │   ├── metadata/     # Entity metadata viewer
│   │   ├── insights/     # Analytics pages
│   │   └── processes/    # Process explorer
│   │
│   ├── components/       # React components
│   │   ├── diagramview/  # Diagram editor (JointJS integration)
│   │   ├── datamodelview/    # Metadata browser
│   │   ├── insightsview/     # Analytics components
│   │   └── shared/           # Shared UI components
│   │
│   ├── contexts/         # React Context providers
│   │   ├── AuthContext.tsx          # Session management
│   │   ├── DatamodelDataContext.tsx # Metadata loading
│   │   ├── DiagramViewContext.tsx   # Diagram state
│   │   └── SettingsContext.tsx      # User preferences
│   │
│   ├── lib/             # Utilities and services
│   │   ├── diagram/     # Diagram serialization, helpers
│   │   ├── Types.ts     # Core type definitions
│   │   └── session.ts   # JWT session management
│   │
│   ├── generated/       # Generated by C# Generator
│   │   └── Data.ts      # Entity metadata (1.5MB)
│   │
│   └── middleware.ts    # Authentication middleware
│
├── Infrastructure/      # Azure Bicep (App Service deployment)
...
DiagramViewContext.addEntity()
  ↓
createEntity() → EntityElement (JointJS)
  ↓
...
**Output**: [Website/.next/standalone/](Website/.next/standalone/) → Deploy to Azure App Service

### CI/CD Pipeline

1. **Build** ([azure-pipelines-build-jobs.yml](azure-pipelines-build-jobs.yml))
   - Build Generator (.NET 8)
   - Run Generator → create Data.ts
...
2. **Deploy** ([azure-pipelines-deploy-jobs.yml](azure-pipelines-deploy-jobs.yml))
   - Deploy Bicep template → App Service + Managed Identity
   - Deploy WebApp.zip
...
## 📈 Insights & Analytics ([app/insights/](Website/app/insights/))
...
- Check DefaultAzureCredential order: Azure CLI → Environment → Managed Identity
...
- Use Smart Layout sparingly (O(n²) complexity)
